<template>
  <v-dialog v-model="pago.estado" v-if="pago.estado" persistent width="700" class="rounded-0 border-card">
    <v-card width="700" class="mx-auto border-card">
      <v-footer color="white" class="mb-0 pb-0">
        <h3 class="mx-auto py-6">Resumen de pago</h3>
        <v-btn icon @click="pago.estado = false"> <v-icon size="30" color="accent"> mdi-close-circle-outline</v-icon> </v-btn>
      </v-footer>
      <v-container>
        <v-data-table height="300" class="elevation-4 border-card mx-4" hide-default-footer :headers="headers" :items="array_order_items">
        ...
        <v-card-actions class="mx-0 my-4 py-0">
          <v-container class="text-end">
            <v-btn @click="generarImpresion()" class="botone mb-1" dark color="boton">
              Descargar comprobante <v-icon class="ml-2">mdi-download-outline</v-icon></v-btn
            >
          </v-container>
        </v-card-actions>
      </v-container>
...
export default {
  ...
  methods: {
    ...
    async generarImpresion() {
      let image = await imageBase64(require("../../assets/image/logoPic40.png"));
      printComanda([...this.array_order_items, this.total], image, this.page.nameClients);
    },
