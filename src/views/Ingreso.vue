    ...
    <v-container class="fill-height pb-0 pt-0 mb-0" fluid>
      <v-row justify="center" align="center">
        <v-col cols="5" md="6" sm="6" xl="6" lg="6" class="py-2">
          <v-img width="415" contain class="mx-auto fill-height" src="../assets/image/logoPic40.png"></v-img>
        </v-col>
          ...
          >
            ...
            <v-form v-model="validacion" ref="form" lazy-validation>
              <v-row justify="center">
                <v-col cols="12" class="px-10 0pb-0 mt-4">
                  <Input_ :field="form.usuario" v-model="user.email" @validateEnter="datoCorreo()" />
                </v-col>
                <v-col cols="12" class="px-10 pb-0 py-0">
                  <Input_ :field="form.password" v-model="user.password" @validateEnter="ingresar()" />
                </v-col>
                <v-col cols="12" class="px-10 pt-0 text-end" @click="cambiar_password.estado = true">
                  <div>
                    <a style="font-size: 15px" class="botone">Olvidé la contraseña</a>
                  </div>
                </v-col>
                <v-col cols="12" class="px-10 pt-0 botone">
                  <v-btn color="boton" dark block @click="ingresar()" elevation="0" large>Ingresar</v-btn>
                </v-col>
              </v-row>
            </v-form>
            <v-row justify="center" class="mt-3">
              ...
              <v-col cols="12" class="px-9 pt-3 text-center">
                <v-btn color="boton" class="botone" @click="register_usuario.estado = true" outlined large>Crear cuenta</v-btn>
              </v-col>
            </v-row>
          </v-card>
          <REGISTRARUSUARIO :register_usuario="register_usuario" class="my-10" />
          <CAMBIARPASSWORD :cambiar_password="cambiar_password" v-if="cambiar_password.estado" class="my-10" />
        </v-col>
      </v-row>
    </v-container>
    <FOOTER />
...
export default {
  ...
  mounted() {
    ...
    this.miembro_votante ? (this.register_usuario.estado = true) : (this.register_usuario.estado = false);
  },
  methods: {
    ...
    async ingresar() {
      ...
      try {
        ...
        if (respuesta == 5) return this.callAlerta("", "info", null, null, this.callbackAlerta, "Pronto se dará inicio a las evaluaciones");
        if (respuesta == "rest") {
          return this.callAlerta("", "info", null, null, this.callbackAlerta, "Evaluaciones finalizadas");
        }
        if ([3].includes(respuesta)) {
          return this.callAlerta("", "info", null, null, this.callbackAlerta, "La selección de ganadores estará habilitada en breve");
        }
        if (respuesta == "no-acceso") return this.callAlerta("CORREO-9", "info");
        if (respuesta === 2) return this.callAlerta("CORREO-1", "email");
        ...
        this.callAlerta("", "info", null, null, this.callbackAlerta, "Las credenciales no son correctas");
      }
    },
