    ...
    <v-container fluid class="bg-color container-channel">
      <v-row>
        <v-col class="d-flex mb-7">
          <v-btn fab color="boton" class="botone mr-3" small elevation="0" sm-d-none @click="retroceder"
        ...
        <v-row class="px-7 mt-">
          <h3>
            En este panel debe votar por los mejores {{ cantidad_votos_ }} postulados, donde podrá evaluar en una puntuación de 1 a
            {{ cantidad_votos_ }} puntos ( 1 calificación más baja y {{ cantidad_votos_ }} calificación mas alta).
          </h3>
      ...
      <v-row class="my-10">
        <v-col cols="12" class="mt-0 pt-0">
          <v-row justify="start" class="mb-8">
            <v-col cols="12" xs="6" sm="6" md="7" lg="7" xl="7" class="py-0 text-start">
              <v-row>
                <v-col cols="10" class="">
                  <h2>
                    Postulados
                    <v-container class="px-0" fluid>
                      <v-checkbox v-model="filtro_category" label="Ordenar"></v-checkbox>
                    </v-container>
                  </h2>
                </v-col>
              </v-row>
          ...
          <v-container fluid class="my-0">
            <v-row v-for="(item, index) in array_categorias" :key="index" class="contenedor-postulados py-1 my-2">
              ...
              <v-col cols="6" xs="6" sm="6" md="2" lg="3" xl="2" class="mr-sm-3">
                <v-btn color="boton" height="40" width="175" class="botone my-0" rounded dark outlined @click="verPrograma(item)">
                  <h3 class="">ver detalles</h3>
                </v-btn>
              </v-col>
              <v-col cols="6" xs="6" sm="6" md="2" lg="3" xl="2" class="">
                <v-container v-if="findVotes(item).position" class="d-flex text-center">
                  <v-row class="text-center" justify="end">
                    <v-rating :value="findVotes(item).position" :length="findVotes(item).position" size="26" dense readonly></v-rating>
                  </v-row>
                </v-container>

                <v-btn v-else color="boton" height="40" width="170" class="botone my-0 mr-10" elevation="0" rounded dark @click="abrirVotar(item)">
                  <h3 class="">Votar</h3>
                </v-btn>
              </v-col>
            </v-row>
...
export default {
  ...
  watch: {
    filtro_category(val) {
      if (val) this.array_categorias.sort((a, b) => this.findVotes(b).position - this.findVotes(a).position);
  ...
  methods: {
    ...mapActions({
      _getPostulationsForCategory: "postulacion/_getPostulationsForCategoryComite",
      ...
      _consultVotesCategory: "comiteTecnico/_consultVotesCategory",
    }),
    async traerConsultas() {
      ...
      const votes = await this._consultVotesCategory(id);
    ...
    async votar(rating) {
      ...
      if (consultarVoto.data.data === null) {
        ...
        const buscar = this.array_categorias.find((e) => e.id == consultarVoto.data.data.postulationId);
